...
def test_write_conf_files(patch_datetime_now):
    ...
    director.constructLookup(lookup_builder, os.path.join(os.path.dirname(__file__), 
        '../builder/test_data/lookups/attacker_tools.yml'))
    lookup = lookup_builder.getObject()
    kv_lookup_builder = SecurityContentBasicBuilder()
    director.constructLookup(kv_lookup_builder, os.path.join(os.path.dirname(__file__), '../builder/test_data/lookups/api_call_by_user_baseline.yml'))
    kv_lookup = kv_lookup_builder.getObject()    
    ...
    adapter.writeObjects([kv_lookup, lookup], output_path, SecurityContentType.lookups)
