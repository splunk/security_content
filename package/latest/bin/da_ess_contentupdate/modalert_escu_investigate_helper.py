...
def process_event(helper, *args, **kwargs):
    ...
    # have been added
    helper.addevent("hello", sourcetype="escu-investigate")
    helper.addevent("world", sourcetype="escu-investigate")
    ...
    helper.log_info("Alert action escu_investigate started.")
    import splunk.rest
    import json
    import time

    # TODO: Implement your alert action logic here
    runSearch = "/servicesNS/nobody/DA-ESS-ContentUpdate/search/jobs?output_mode=json&output=-1"
    ...
    try:
        ...
        for event in events:
            ...
            # Endpoint searches have user field, but some investigative searches need src_user
            if 'user' in event and 'src_user' not in event:
                event['src_user'] = event['user']

            helper.log_info("Gathering investigative search data")
            get_investigative_search_data = "| rest /services/saved/searches splunk_server=local count=0 | search * [| rest /services/configs/conf-analytic_stories splunk_server=local count=0 | search detection_searches=\"*" + event['search_name'][:-7] + "*\" | spath input=investigative_searches path={} output=invs | mvexpand invs | table invs | sort invs | uniq invs | rename invs as title] | table title search, action.escu.earliest_time_offset, action.escu.latest_time_offset, action.escu.fields_required"
            #add this to our post data for the splunk search
            pdata = {'search': get_investigative_search_data}
            ...
            later_offset = {}
            fields_required = {}
            investigative_search_data = json.loads(content)
            for invs_data in investigative_search_data['results']:
                invs = invs_data['title']

                searches_to_run[invs] = invs_data['search']
                earlier_offset[invs] = int(invs_data['action.escu.earliest_time_offset'])
                later_offset[invs] = int(invs_data['action.escu.latest_time_offset'])
                fields_required[invs] = invs_data['action.escu.fields_required']

            helper.log_info("Running Investigative Searches")
            for search_name, search in searches_to_run.iteritems():
                ...
                head, content = splunk.rest.simpleRequest(runSearch, sessionKey=helper.settings["session_key"], postargs=invs_sdata, method='POST')
                #get our search ID/sid
                ...
                if isDone:
                    head, investigative_content = splunk.rest.simpleRequest(pollSearch + sid_data['sid'] + "/results?output_mode=json", sessionKey=helper.settings["session_key"], method='GET')
                    investigative_data = json.loads(investigative_content)
                    for new_data in  investigative_data['results']:
                        new_data['search_name'] = search_name
                        results = json.dumps(new_data)
                        helper.addevent(str(results), sourcetype="escu-investigate")
                else:
                    new_data = {}
                    new_data['search_name'] = search_name
